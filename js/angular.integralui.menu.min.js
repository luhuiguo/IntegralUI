...
updateLayout:function(e,g){b.$broadcast(e+"-update-layout",g)}}}]).controller("IntegralUIMenuController",["$scope","$element","IntegralUIInternalService","IntegralUIDataService","IntegralUIMenuService","$timeout",function(b,y,e,g,m,k){var d=this;d.hoverItem=null;d.defaultStyle={general:{disabled:"iui-menu-disabled",normal:"iui-menu"},block:{general:"iui-menu-block",marker:{top:"iui-menu-marker-top",left:"iui-menu-marker-left",bottom:"iui-menu-marker-bottom",right:"iui-menu-marker-right",rtlTop:"iui-menu-marker-top-rtl",
...
d.options.dataFields.icon,d.options.dataFields.id,d.options.dataFields.hasChildren,d.options.dataFields.pid,d.options.dataFields.style,d.options.dataFields.text,d.options.dataFields.type,d.options.dataFields.value];0==a&&c.push(d.options.dataFields.items);return JSON.stringify(e,c,b)};this.loadData=function(c,a,e,m){d.allowEvents=!1;d.updateDataFields(e);var k=[],q=d.options.dataFields;d.dataObj.clear(a);if(c)if(0!=m){var n=[];c.forEach(function(c,a){var d=c[q.pid];n[d]?(n[d][q.items]||(n[d][q.items]=
[]),n[c[q.id]]=c,n[d][q.items].push(c)):(n[c[q.id]]=c,k.push(c))});n.length=0}else k=c;if(a)for(a[d.options.dataFields.items]=k,c=0;c<a[d.options.dataFields.items].length;c++)a[d.options.dataFields.items][c][d.options.dataFields.pid]=a[d.options.dataFields.id];else angular.isDefined(b.items)&&(b.items=k);d.dataObj=new g({objects:b.items,events:d.dataEvents,fields:d.getDataFields(d.options.dataFields)});d.updateLayout(a);d.allowEvents=!0};this.callClose=function(c){return angular.isDefined(b.events)&&
...
return{general:b,content:f,loadIcon:g,separator:e.isFieldAvailable(a.separator,"iui-menu-item-separator")}}return d.options.controlStyle.item},w=function(a){if(a){var b;b=(b=a.general)?{disabled:e.isFieldAvailable(b.disabled,"iui-menu-item-root-disabled"),focused:e.isFieldAvailable(b.focused,"iui-menu-item-root-focused"),normal:e.isFieldAvailable(b.normal,"iui-menu-item-root"),hovered:e.isFieldAvailable(b.hovered,"iui-menu-item-root-hovered"),selected:e.isFieldAvailable(b.selected,"iui-menu-item-root-selected"),
...
f.general.normal:d.options.controlStyle.root.general.normal}}else return f?f:d.options.controlStyle.root};var A=function(a){return a?e.isString(a)?a:{disabled:e.isFieldAvailable(a.disabled,"iui-menu-disabled"),normal:e.isFieldAvailable(a.normal,"iui-menu")}:d.defaultStyle.general};this.updateControlStyle=function(a){d.options.controlStyle=a?{general:A(a.general),block:E(a.block),item:u(a.item),root:w(a.root)}:{general:A(d.defaultStyle.general),block:E(d.defaultStyle.block),item:u(d.defaultStyle.item),
root:w(d.defaultStyle.root)}}}]).directive("iuiMenu",["$compile","$window","$timeout","$interval","IntegralUIInternalService",function(b,y,e,g,m){return{restrict:"EA",controller:"IntegralUIMenuController",transclude:!0,replace:!0,template:'<div class="iui-menu" data-element="menu"><ul class="iui-menu-block"></ul></div>',scope:{controlStyle:"=",enabled:"=",itemIcon:"=",items:"=",name:"@",openOnHover:"=",options:"=?",displayMode:"@",rtl:"=",showDropdownIcon:"=",showIcons:"=",close:"&",enabledChanged:"&",
events:"=?",itemAdded:"&",itemAdding:"&",itemClick:"&",itemHover:"&",itemRemoved:"&",itemRemoving:"&",open:"&",styleChanged:"&"},link:function(k,d,B,a,E){var u=d.children().eq(0),w=function(a){d.unbind("mouseleave");a=a?a.find("li"):u.children();if(0<a.length)for(var c=0;c<a.length;c++)currentElem=angular.element(a[c]),currentElem.unbind("click mouseenter mouseleave touchstart")},A=function(b){if(1==a.options.enabled){var e=null;d.bind("mouseleave",function(a){a.stopPropagation()});b=b?b.find("li"):
u.children();if(0<b.length)for(var F=null,f=0;f<b.length;f++)F=angular.element(b[f]),(e=a.getItemFromElem(F))&&a.isItemEnabled(e)&&c(F)}},c=function(c){c&&(c.bind("click",function(c){var b=a.getItemFromElem(this);b&&0!=a.callItemClick(b,m.getMousePos(c))&&(a.isRoot(b)?(angular.element(this),t?(a.closeItem(b),a.isRoot(b)&&(t=!1,f=null)):a.openItem(b)):z(b)||q());c.stopPropagation()}),c.bind("mouseenter",function(c){var b=a.getItemFromElem(this);b&&(angular.element(this),a.hoverItem=b,a.refresh(null,
!0),a.callItemHover(b),a.options.openOnHover||!a.isRoot(b)?(n&&e.cancel(n),n=e(function(){n&&a.openItem(b)},500)):t&&b!=f&&(f&&a.closeItem(f),a.openItem(b)));c.stopPropagation()}),c.bind("mouseleave",function(c){a.hoverItem=null;var b=a.getItemFromElem(this);if(b&&(a.refresh(null,!0),!a.options.openOnHover&&f&&(f[a.options.dataFields.selected]=!0,a.refresh(f)),a.options.openOnHover||!a.isRoot(b)))var d=e(function(){a.closeItem(b);e.cancel(d)},250);n&&e.cancel(n);n=null}),c.bind("touchstart",function(c){c.preventDefault();
var b=m.getTouchData(c);b&&0<b.length&&((b=a.getItemFromElem(this))&&0!=a.callItemClick(b,m.getMousePos(c))&&(a.isRoot(b)?(angular.element(this),t?(a.closeItem(b),a.isRoot(b)&&(t=!1,f=null)):a.openItem(b)):z(b)||q()),c.stopPropagation())}))};angular.element(y).bind("click",function(a){q()});angular.element(y).bind("keydown",function(b){switch(b.keyCode){case 27:a.closeItem(f),t=!1,f=null,a.refresh(null,!0)}});var t=!1,f=null,r=[],C=function(b){return b?(b=b[a.options.dataFields.items],void 0!=b&&
0<b.length):!1};a.openItem=function(b){var c=a.getElemFromItem(b);if(b&&z(b)&&p&&c){var d=c.find("ul");if(0<d.length&&0!=a.callOpen(b)){d=angular.element(d.eq(0));w(d);C(b)&&(c.removeClass(a.options.controlStyle.block.marker.expand.down),c.removeClass(a.options.controlStyle.block.marker.expand.left),c.removeClass(a.options.controlStyle.block.marker.expand.right),c.removeClass(a.options.controlStyle.block.marker.expand.rtlDown));t=!0;(f=a.isRoot(b)?b:f)&&(f[a.options.dataFields.selected]=!0);0>r.indexOf(f)&&
r.unshift(f);for(var g=0;g<r.length;g++)r[g]!=f&&(r[g][a.options.dataFields.selected]=!1,a.closeItem(r[g]));r.splice(1,r.length-1);if(C(b)){g=a.getItemIndex(b);if(0<=g){var v=0,l="auto",h="auto",k=angular.element(c.parent()),n=m.getPadding(c[0]),q=m.getPadding(k[0]);a.options.rtl?"vertical"==a.options.displayMode?a.isRoot(b)?(v=0,h=-(k[0].offsetWidth+16)):(v-=q.top,l=k[0].offsetWidth-8):a.isRoot(b)?(v=c[0].offsetTop-n.bottom+c[0].offsetHeight,l=0):(v-=q.top,l=k[0].offsetWidth-8):"vertical"==a.options.displayMode?
(v=a.isRoot(b)?0:v-q.top,h=k[0].offsetWidth-8):a.isRoot(b)?(v=c[0].offsetTop-n.bottom+c[0].offsetHeight,h=0):(v-=q.top,h=k[0].offsetWidth-8);c="display:block;"+("top:"+v+"px;");"auto"!=h&&(c+="left:"+h+"px;");"auto"!=l&&(c+="right:"+l+"px;");(b=D(a.getParent(b)))&&b.data&&g<b.data.length&&(b.data[g].inlineBlockStyle=c,p.$apply())}e(function(){A(d);I()},1)}}}};a.closeItem=function(b){var c=a.getElemFromItem(b);if(b&&p&&c){var d=c.find("ul");0<d.length&&0!=a.callClose(b)&&(d=angular.element(d.eq(0)),
w(d),z(b)&&(a.isRoot(b)?"hovered"==a.getObjState(b)&&(a.options.rtl?c.addClass(a.options.controlStyle.block.marker.expand.rtlDown):c.addClass(a.options.controlStyle.block.marker.expand.down)):a.options.rtl?c.addClass(a.options.controlStyle.block.marker.expand.left):c.addClass(a.options.controlStyle.block.marker.expand.right)),c=a.getItemIndex(b),0<=c&&(b=D(a.getParent(b)))&&b.data&&c<b.data.length&&(b.data[c].inlineBlockStyle="display:none;",p.$apply()))}};var q=function(){a.closeItem(f);t=!1;f=null;
a.refresh(null,!0);n&&e.cancel(n);n=null},n=null,p=null,z=function(b){if(b){var c=b[a.options.dataFields.items];return b[a.options.dataFields.hasChildren]||c&&0<c.length}return!1},K=function(b,c,d){var e="";b[a.options.dataFields.icon]||""==b[a.options.dataFields.icon]?e=b[a.options.dataFields.icon]:a.options.itemIcon&&(e=a.options.itemIcon);return{icon:e,iconVisible:0!=a.options.showIcons&&(null!=b.icon||null!=a.options.itemIcon),index:c,inlineBlockStyle:"display:none",item:b,style:{},visible:d}},
G=function(b){var c=[];if(b)for(var d=0;d<b.length;d++)c.push(K(b[d],a.getItemFullIndex(b[d])));return c},I=function(){var a=u.find("li");if(0<a.length)for(var b=0;b<a.length;b++)for(var c=angular.element(a[b]),d=c.children(),e=0;e<d.length;e++){var f=angular.element(d[e]);f[0].attributes["data-element"]&&"load-icon"==f[0].attributes["data-element"].value.toString()&&f.css("top",(c[0].clientHeight-f[0].offsetHeight)/2+"px")}},L=function(){var a=u.find("li");if(0<a.length)for(var b=0;b<a.length;b++){var c=
angular.element(a[b]);c&&c.attr("data-index",b)}},J=function(a,b){var c=null;if(a&&b)for(var d=0;!c&&d<a.length;)c=a[d].item==b?a[d]:J(a[d].data,b),d++;return c},D=function(a){return a?J(p.data,a):p},M=function(c){var f=e(function(){w();a.options.rtl?d.css("direction","rtl"):d.css("direction","ltr");c||(u.empty(),p&&p.$destroy());e.cancel(f)},1),g=e(function(){var d="";if(c){var f=D(c);if(f){var l=c[a.options.dataFields.items];if(l&&0<l.length){f.data=G(l);a.refresh();p.$apply();for(var h=0;h<l.length;h++)d+=
H(h,l[h],f.scopeRef,f.data);""!=d&&(f=a.getBlockElemFromItem(c))&&(f.empty(),f.append(b(d)(p)))}L()}}else{p=k.$new();p.data=G(a.currentList);a.refresh();p.$apply();for(f=0;f<a.currentList.length;f++)d+=H(f,a.currentList[f],"data",p.data);""!=d&&u.append(b(d)(p))}A();var m=e(function(){I();e.cancel(m)},1);e.cancel(g)},1)},H=function(b,c,d,e){a.getItemFullIndex(c);var f='<li iui-class="{{'+d+"["+b+'].style.general}}" data-index="'+a.getItemFullIndex(c)+'">';switch(c.type){case "separator":f+='<hr iui-class="{{'+
...
t&&C(b)||1!=a.options.showDropDownIcon&&"hovered"!=g||a.options.loadItem||(l.general=a.options.rtl?l.general+(" "+a.options.controlStyle.block.marker.expand.left):l.general+(" "+a.options.controlStyle.block.marker.expand.right))):(h!=a.options.controlStyle.root.general.normal&&(l.general+=" "+h),!f||(1!=a.options.showDropDownIcon||0!=b[a.options.dataFields.selected])&&t&&C(b)||1!=a.options.showDropDownIcon&&"hovered"!=g||a.options.loadItem||(l.general=a.options.rtl?l.general+(" "+a.options.controlStyle.block.marker.expand.rtlDown):
